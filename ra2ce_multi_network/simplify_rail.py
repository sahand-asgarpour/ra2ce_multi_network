...
def get_rail_network_with_terminals(
    network_gdf: gpd.GeoDataFrame, aggregation_range: float
    ...
    network.nodes = _aggregate_terminal_nodes(
        network, aggregation_range=aggregation_range
    )
...
def get_rail_network_with_given_terminals(
    network_gdf: gpd.GeoDataFrame, od_file: Path, network=None
) -> snkit.network.Network:
    if network is None:
        network = _make_network_from_gdf(network_gdf=network_gdf)
    network.set_crs(crs="EPSG:4326")
    ...
    od_gdf, graph = add_od_nodes(
        od=gpd.read_file(od_file), graph=graph, crs=pyproj.CRS("EPSG:4326")
    )
...
def _make_network_from_gdf(network_gdf: gpd.GeoDataFrame) -> snkit.network.Network:
    ...
    net.edges.from_id = net.edges.from_id.str.extract(r"(\d+)", expand=False).astype(
        int
    )
...
def _nx_to_network(
    g: Union[Graph, MultiGraph, MultiDiGraph],
    node_id_column_name="id",
    edge_from_id_column="from_id",
    edge_to_id_column="to_id",
    default_crs: CRS = CRS.from_epsg(4326),
) -> snkit.network.Network:
...
def _detect_possible_terminals(network: snkit.network.Network) -> snkit.network.Network:
    ...
    possible_terminal = (
        network.edges[["from_id", "to_id", "service"]]
        .apply(
            lambda x: _check_terminal_criteria(
                x["from_id"], x["to_id"], x["service"], hanging_nodes
            ),
            axis=1,
        )
        .dropna()
    ...
    network.nodes["possible_terminal"] = network.nodes["id"].apply(
        lambda x: 1 if x in possible_terminal else 0
    )
    return network


def _aggregate_terminal_nodes(
    network: snkit.network.Network, aggregation_range: float
) -> snkit.network.Network:
    ...
    def update_node_gdf(
        ...
        terminal_collection: gpd.GeoDataFrame,
        considered_node_ids: list,
    ) -> gpd.GeoDataFrame:
        if len(terminal_collection["id"].to_numpy().tolist()) == 1:
            return node_gdf
        if not any(
            node_id in considered_node_ids
            for node_id in terminal_collection["id"].to_numpy().tolist()
        ):
            ...
            aggregate_terminal_gdf = get_centroid_terminal_gdf(
                terminal_to_aggr_again_gdf
            )
            to_aggregate_again = terminal_to_aggr_again_gdf[
                terminal_to_aggr_again_gdf["terminal_collection"].apply(
                    lambda x: len(x) > 1
                )
            ]["id"].tolist()
    ...
    def get_centroid_terminal_gdf(
        _gdf: gpd.GeoDataFrame,
    ) -> Union[gpd.GeoDataFrame, None]:
        aggregation_range_degree = _km_distance_to_degrees(aggregation_range)
        if _gdf.empty:
            return None
        _centroid_terminal_collection = _gdf[
            "geometry"
        ].unary_union.centroid  # the point(its gdf) does not have crs
        transformer = pyproj.Transformer.from_crs(
            "epsg:4326", pyproj.CRS("epsg:4326"), always_xy=True
        )
        projected_centroid_terminal_collection = Point(
        ...
        return gpd.GeoDataFrame(
            {
                "id": new_id,
                "geometry": Point(
                    projected_centroid_terminal_collection.x,
                ...
                "terminal_collection": [
                    {
                        term_col
                        for _id in _terminal_collection_ids
    ...
    if aggregation_range == 0:
        node_gdf["terminal_collection"] = node_gdf.apply(
            lambda row: (
                list(row["terminal_collection"])
                if isinstance(row["terminal_collection"], set)
                else row["possible_terminal"]
            ),
            axis=1,
        )
        return node_gdf
    ...
    node_gdf["buffer"] = node_gdf.apply(
        lambda row: (
            row.geometry.buffer(aggregation_range)
            if row["possible_terminal"] == 1
            else None
        ),
        axis=1,
    )
    ...
    node_gdf["terminal_collection"] = node_gdf.apply(
        lambda row: (
            row["terminal_collection"]
            if isinstance(row["aggregate"], int) and row["aggregate"] == 1
            else "n.a."
        ),
        axis=1,
    )
...
def _add_demand_edge(network: snkit.network.Network) -> snkit.network.Network:
    ...
    aggregate_demand_nodes = node_gdf[
        (node_gdf["aggregate"] == 1) & (node_gdf["terminal_collection"].str.len() > 1)
    ]
    ...
    new_edges_data = [
        {
            "geometry": LineString(
                [
                    ...
                    (
                        node_gdf.loc[child_ter_id, "geometry"].x,
                        node_gdf.loc[child_ter_id, "geometry"].y,
                    ),
                ]
            ),
            "demand_edge": int(1),
            ...
            **{
                ...
                if column not in ["geometry", "id", "from_id", "to_id"]
            },
        }
...
def _get_demand_edge_attributes(
    network: snkit.network.Network,
) -> snkit.network.Network:
    edge_cols = network.edges.columns.tolist()
    node_cols = network.nodes.columns.tolist()
    network_x = _to_networkx(
        network=network,
        directed=False,
        node_attributes=node_cols,
        edge_attributes=edge_cols,
    )
    aggregate_demand_nodes = network.nodes[
        (network.nodes["aggregate"] == 1)
    ...
    for _, row in aggregate_demand_nodes.iterrows():
        for child_ter_id in row["terminal_collection"]:
            ...
            edge_to_update_info = [
                edge for edge in neighbor_edges if edge[2]["demand_edge"] == 1
            ]

            if len(edge_to_update_info) == 1 and len(neighbor_edges) > 1:
                edge_to_update = edge_to_update_info[0]
                reference_edge = [
                    edge for edge in neighbor_edges if edge != edge_to_update
                ][0]

                attributes_to_update = [
                    attr for attr, value in edge_to_update[2].items() if value is None
                ]
                edge_to_update[2].update(
                    {
                        attribute: reference_edge[2][attribute]
                        for attribute in attributes_to_update
                    }
                )

                network_x[edge_to_update[0]][edge_to_update[1]].update(
                    edge_to_update[2]
                )
            else:
                raise ValueError(
                    """Invalid edge graph. There is more or less than one demand node connected or 
...
def _add_nodes_to_nx(
    g: Union[Graph, MultiDiGraph], net: snkit.network.Network, node_atts: list
):
    ...
    for _, row in net.nodes.iterrows():
        node_id = row["id"]
        attributes = (
            row[node_atts].to_dict() if node_atts else {"geometry": row["geometry"]}
        )
        g.add_node(node_id, **attributes)


def _add_links_to_nx(
    g: Union[Graph, MultiDiGraph], net: snkit.network.Network, edge_atts: list
):
    if (
        "from_id" not in net.edges.columns.tolist()
        or "to_id" not in net.edges.columns.tolist()
    ):
    ...
    for _, row in net.edges.iterrows():
        ...
        attributes = (
            row[edge_atts].to_dict() if edge_atts else {"geometry": row["geometry"]}
        )
        g.add_edge(u, v, **attributes)
...
def _to_snkit_network(netx: Union[Graph, MultiDiGraph]) -> snkit.network.Network:
    ...
    edge_dict = {
        ind: {**edges[2], "from_id": edges[0], "to_id": edges[1]}
...
def _check_terminal_criteria(
    from_node_id: int, to_node_id: int, edge_property: str, hanging_nodes: np.ndarray
) -> int:
...
def _get_merge_edge_paths(
    ...
    def _merge(
        gdf: GeoDataFrame, by: list, _aggfunc: dict, ntw: snkit.network.Network
    ) -> GeoDataFrame:
        def _get_merged_in_a_loop(_merged: GeoDataFrame) -> GeoDataFrame:
            # 2.1.2. pick one with one intersection point
            start_path_extrms = [
                gdf_node_slice[gdf_node_slice["degree"] > 2].iloc[0].id
            ]
            ...
            _merged.to_id = [
                end_path_extremity for end_path_extremity in end_path_extrms
            ]
            return _merged

        def _get_merged_multiple_demand_edges(
            _merged: GeoDataFrame, path_extrms_nod_ids: set
        ) -> GeoDataFrame:
            _mrgd = _get_split_edges_info(_merged)
            _mrgd.from_id = _mrgd.apply(
                lambda row: _get_node_id(row, "from_id", path_extrms_nod_ids), axis=1
            )
            _mrgd.to_id = _mrgd.apply(
                lambda row: _get_node_id(row, "to_id", path_extrms_nod_ids), axis=1
            )
            return _mrgd
        ...
        def _get_split_edges_info(_merged: GeoDataFrame) -> tuple:
            ...
            dem_nod_ids = [
                i
                for i in set(gdf.from_id.tolist() + gdf.to_id.tolist())
                if (
            ...
            for dem_nod_id in dem_nod_ids:
                for part in split_parts:
                    part_splits, split_edges_gdf = _split(
                        part, dem_nod_id, split_edges_gdf
                    )
        ...
        def _split(
            _line_geom: Union[MultiLineString, LineString],
            dem_nod_id: int,
            splits_gdf: GeoDataFrame,
        ) -> tuple:
            # used for the cases where demand nodes exist in the to-be-merged paths
            ...
            if _line_geom.contains(dem_nod_geom):
                if isinstance(_line_geom, MultiLineString):
                    coords = [
                        linemerge(_line_geom).coords[0],
                        linemerge(_line_geom).coords[-1],
                    ]
                else:
                    coords = [_line_geom.coords[0], _line_geom.coords[-1]]
                    ...
                    part_gdf = gpd.GeoDataFrame(
                        {
                            ...
                            "to_id": -1,
                            **_merged.drop(
                                columns=["geometry", "id", "from_id", "to_id"]
                            ),
                        }
                    )
                else:
                    ...
                    part_gdf = gpd.GeoDataFrame(
                        {
                            ...
                            "to_id": splt_edgs[
                                ...
                                lambda row: (
                                    row.to_id if row.to_id != -1 else dem_nod_id
                                ),
                                axis=1,
                            ),
                            **_merged.drop(
        ...
        def _get_node_id(r: GeoSeries, attr: str, path_extrms_nod_ids: set) -> int:
            ...
            if r[attr] == -1:
                for path_extremities_node_id in path_extrms_nod_ids:
                    path_extremities_node_geom = ntw.nodes[
                        ntw.nodes.id == path_extremities_node_id
                    ].geometry.iloc[0]
        ...
        def _get_merged_one_or_none_demand_edges(
            _merged, path_extrms_nod_ids: set
        ) -> GeoDataFrame:
            _start_edges = gdf[gdf["intersections"].apply(lambda x: len(x) == 1)]
            if ("demand_edge" in gdf.columns) and (
                len(gdf[gdf["demand_edge"] == 1])
            ) == 1:
                _start_edge = _start_edges[_start_edges.demand_edge == 1].iloc[0]
            elif ("demand_edge" not in gdf.columns) or (
                len(gdf[gdf["demand_edge"] == 1])
            ) != 1:
            ...
            if len(gdf[gdf["intersections"].apply(lambda x: len(x) == 1)]) == 0:
                ...
                if len(gdf_node_slice[gdf_node_slice["degree"] > 2]) == 0:
                    ...
                    if "demand_edge" in gdf.columns:
                        warnings.warn(
                            f"""'This sub-graph had these demand nodes {(
                                gdf[gdf.demand_edge == 1].from_id.tolist() +
                                gdf[gdf.demand_edge == 1].to_id.tolist()
                        )}"""
                        )
                    return gpd.GeoDataFrame(
                        data=None, columns=ntw.edges.columns, crs=ntw.edges.crs
                    )
                    ...
                    if (
                        ...
                        or len(gdf[gdf["demand_edge"] == 1]) == 0
                    ):
                        ...
                        "demand_edge" in gdf.columns
                        and len(gdf[gdf["demand_edge"] == 1]) > 0
                    ):
                        demand_node_ids = [
                            i
                            for i in set(gdf.from_id.tolist() + gdf.to_id.tolist())
                        ...
                        if len(demand_node_ids) > 1:
                            ...
                            _merged = _get_merged_multiple_demand_edges(
                ...
                if ("demand_edge" in gdf.columns) and (
                    ...
                    "demand_edge" in gdf.columns
                    and len(gdf[gdf["demand_edge"] == 1]) <= 1
                ) or ("demand_edge" not in gdf.columns):
    ...
    if len(grouped_edges.groups) == 1:
        merged_edges = _merge(
            gdf=edges, by=excluded_edge_types, _aggfunc=aggfunc, ntw=net
        )
    else:
...
def _merge_edges(
    ...
    updated_edges = _get_merged_edges(
        paths_to_group=edge_paths, by=by, aggfunc=aggfunc, net=net
    )
...
def _simplify_tracks(
    net: snkit.network.Network, buffer_distance: float, hole_area_threshold: float
):
...
def _drop_hanging_nodes(net, tolerance=1):
    ...
    def _get_edges_with_hanging_nodes(
        egs: GeoDataFrame, hng_nds: np.ndarray
    ) -> GeoDataFrame:
    ...
    if "demand_edge" in deg_ed.columns:
        while (
            len(deg_ed[deg_ed["demand_edge"] != 1]) > 0
        ):  # while there are edges with deg 1 node, excluding dem_edges
...
def _unified_buffer_tracks(
    net: snkit.network.Network, buffer_distance_km: float
) -> GeoDataFrame:
    buffer_distance_deg = _km_distance_to_degrees(buffer_distance_km)
    buffered_data = net.edges.set_crs(net.nodes.crs).geometry.buffer(
        distance=buffer_distance_deg, cap_style="square"
    )
...
def get_largest_polygon(input_poly_gdf: GeoDataFrame) -> GeoDataFrame:
    ...
    return gpd.GeoDataFrame(
        {"id": [1], "geometry": MultiPolygon([largest_polygon])},
        geometry="geometry",
        crs="EPSG:4326",
    )
...
def _remove_small_holes(
    input_poly_gdf: GeoDataFrame, hole_area_threshold: float
) -> GeoDataFrame:
    """
    ...
    return gpd.GeoDataFrame(
        {"geometry": [MultiPolygon([new_polygon for new_polygon in new_polygons])]},
        geometry="geometry",
        crs=input_poly_gdf.crs,
    )
...
def _triangulate_polygon(poly_gdf: gpd.GeoDataFrame) -> dict:
    ...
    if isinstance(poly_geom, MultiPolygon):
        for p in poly_geom.geoms:
            ...
            _triangulation_data = _update_dict(
                _triangulation_data, new_triangulation_data
            )
    else:
...
def _filter_triangles(
    polygon: Union[Polygon, MultiPolygon], triangulation_result: dict
) -> dict:
    ...
    for _, tri in enumerate(triangulation_result["triangles"]):
        ...
        _tri_sides = [
            LineString([_tri_coords[i], _tri_coords[(i + 1) % 3]])
            for i, vertex_id in enumerate(tri)
        ]

        edge_polygon_intersections = {edge.intersection(polygon) for edge in _tri_sides}
        sum_edges_touching_triangulated_boundary = sum(
            isinstance(edge_polygon_intersection, (Point, MultiPoint))
            for edge_polygon_intersection in edge_polygon_intersections
        )
        sum_edges_within_polygon = sum(
            polygon.contains(edge_polygon_intersection)
            for edge_polygon_intersection in edge_polygon_intersections
        )
        if (
            sum_edges_touching_triangulated_boundary >= 2
            and sum_edges_within_polygon == 0
        ):
            continue
        elif sum_edges_touching_triangulated_boundary != 3 and (
            3 > sum_edges_within_polygon > 0
        ):
            _triangle_types.append("normal")
        elif (
            sum_edges_touching_triangulated_boundary == 1
            and sum_edges_within_polygon == 0
        ):
            _triangle_types.append("normal")
...
def _create_triangle_gdf(
    _triangulation_data: dict, polygon: GeoDataFrame
) -> GeoDataFrame:
    def create_triangle_row(
        idx: int, triangle: Polygon, _triangulation_data: dict
    ) -> dict:
        return {
            "id": idx,
            "geometry": triangle,
    ...
    rows = (
        create_triangle_row(idx, triangle, _triangulation_data)
        for idx, triangle in enumerate(_triangulation_data["triangle_geometries"])
    )
...
def _identify_intersected_track_edges(
    triangle: list, net: snkit.network.Network
) -> list:
...
def cut_track_edges_at_triangle_sides(
    track_edges: GeoDataFrame, triangulation_data: dict
) -> GeoDataFrame:
    cut_track_edges = []
    for tri in triangulation_data["triangles"]:
        cut_result = gpd.overlay(
            track_edges, gpd.GeoDataFrame(geometry=[tri]), how="intersection"
        )
